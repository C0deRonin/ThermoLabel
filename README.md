...
Веб-приложение для аннотирования тепловых изображений: bounding box и полигоны, температурные классы, палитры, экспорт в **YOLO / COCO / Pascal VOC**, сохранение проектов в **PostgreSQL**, дамп БД из интерфейса. **Запуск только через Docker.** Инициализация и перенос данных — **только по дампу** (SQL/бинарный дамп PostgreSQL).

> В интерфейсе может отображаться название **Infralabel** — это тот же продукт (брендинг UI); репозиторий и образы Docker — **ThermoLabel**.

## Скриншоты интерфейса (v1.0.0)
...
*Файлы лежат в [`docs/images/`](docs/images/). При необходимости замените их своими скриншотами (PNG/JPEG) — см. [`docs/images/README.md`](docs/images/README.md).*

## Возможности

- **Аннотация:** Bounding Box, полигон; тепловая карта; зум.
- **Классы:** диапазоны температур, цвета; вкладка управления классами.
- **Экспорт:** YOLO, COCO JSON, Pascal VOC; при сохранённом проекте — запись экспорта в БД (`project_exports`).
- **Проекты:** список из PostgreSQL, сохранение/загрузка, переименование.
- **БД:** полный дамп или «только данные» для слияния; импорт с опцией очистки перед полным дампом.
...
├── docs/
│   ├── images/               # Скриншоты для README (PNG)
│   └── ...
├── docker-compose.yml
