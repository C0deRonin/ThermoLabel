...
const getClassId = (name, classes) => {
  ...
  return Math.max(
    0,
    cls.findIndex((c) => c.name === name || c.id === name)
  );
};
...
export function exportYOLO(annotations, W, H, classes = []) {
  ...
  return (annotations || [])
    ...
    .map((a) => {
      ...
      return `${c} ${(x / safeW).toFixed(6)} ${(y / safeH).toFixed(6)} ${(width / safeW).toFixed(6)} ${(height / safeH).toFixed(6)}`;
    })
    .join("\n");
}
    ...
    annotations: (annotations || []).map((a, i) => {
      if (a.type === "polygon") {
        return {
          id: i + 1,
          image_id: 1,
          category_id: getClassId(safeLabel(a), cls) + 1,
...
export function downloadFile(content, filename, mime = "text/plain") {
  ...
  if (hasObjectUrl && typeof URL.revokeObjectURL === "function") {
