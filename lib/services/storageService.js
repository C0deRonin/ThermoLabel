...
const deserializeProject = (project) => {
  ...
  if (project.image_encoding === "base64-u8" && typeof project.image_data === "string") {
...
const trimOldProjectsUntilFits = (serializedProject) => {
  ...
  while (list.length > 0) {
    ...
    try {
      localStorage.removeItem(`${KEYS.projectPrefix}${oldest.id}`);
      list = list.slice(0, -1);
...
const storageService = {
  ...
  loadProject(id) {
    return deserializeProject(read(`${KEYS.projectPrefix}${id}`, null));
  },
